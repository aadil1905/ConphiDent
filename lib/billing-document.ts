...
export type BillingDocumentInput = {
  clinic: { name: string; brandName?: string | null; logoUrl?: string | null; accentColor?: string | null; address?: string | null; phone?: string | null; email?: string | null; gstin?: string | null; registrationNumber?: string | null; invoiceFooter?: string | null; paymentDetails?: string | null };
  invoice: { invoiceNumber: string; documentType: string; issueDate: Date; dueDate?: Date | null; subtotalAmount?: number | null; discountAmount: number; taxAmount: number; totalAmount: number; status: string; notes?: string | null; terms?: string | null; immutableSnapshot?: unknown; patient: { fullName: string; phone: string; email?: string | null }; payments: { amount: number; method: string; paidAt: Date; status?: string; receiptNumber?: string | null; referenceNumber?: string | null }[]; lineItems: { id: number; description: string; quantity: number; unitPrice: number; discount: number; taxPercent: number; lineTotal: number }[] };
};
...
export function buildInvoiceDocument(input: BillingDocumentInput) {
  ...
  return {
    ...
    accentColor: frozen ? text(snapshotClinic?.accentColor, "#0e7490")! : input.clinic.accentColor || "#0e7490",
