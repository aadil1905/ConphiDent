...
export const ROLES = ["OWNER", "ADMINISTRATOR", "DENTIST", "RECEPTIONIST", "BILLING", "ASSISTANT", "INVENTORY", "LAB"] as const;
export const PASSWORD_MIN_LENGTH = 12;

/** A deliberately boring policy that works with the existing scrypt password store. */
export function passwordPolicyError(password: string) {
  if (password.length < PASSWORD_MIN_LENGTH) return `Use at least ${PASSWORD_MIN_LENGTH} characters.`;
  if (!/[a-z]/.test(password) || !/[A-Z]/.test(password) || !/\d/.test(password)) return "Use upper- and lower-case letters plus a number.";
  return null;
}
/** Durable, hashed-subject throttle for authentication flows. */
export async function consumeSecurityRateLimit(scope: string, subject: string, max: number, windowMs: number) {
  const subjectHash = createHash("sha256").update(subject.trim().toLowerCase()).digest("hex");
  const now = new Date();
...
export const getCurrentUser = cache(async () => {
  ...
  if (!session || session.expiresAt <= new Date() || !session.user.active || session.user.clinic.status !== "ACTIVE") {
    if (session?.expiresAt && session.expiresAt <= new Date()) await prisma.session.delete({ where: { id: parsed.id } }).catch(() => undefined);
    return null;
  }
  return session.user;
});
