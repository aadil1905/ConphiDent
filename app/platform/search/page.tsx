...
export default async function PlatformSearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  return <main className="space-y-6 pb-12"><header><p className="platform-eyebrow">Control Center</p><h1 className="mt-1 text-3xl font-bold">Search</h1><p className="mt-2 text-sm text-muted-foreground">Results are restricted to the permissions of your current platform role.</p></header>
    <form className="flex max-w-2xl gap-2"><label className="sr-only" htmlFor="q">Search</label><input id="q" name="q" defaultValue={q} maxLength={80} placeholder="Search permitted records" className="h-11 min-w-0 flex-1 rounded-lg border bg-card px-3"/><button className="rounded-lg bg-primary px-4 text-sm font-semibold text-white">Search</button></form>
...
function Empty({ text }: { text: string }) { return <section className="rounded-xl border border-dashed bg-card p-8 text-sm text-text-muted">{text}</section>; }
function Results({ title, items }: { title: string; items: React.ReactNode[] }) { return <section className="rounded-xl border bg-card"><h2 className="border-b px-5 py-4 font-bold text-heading">{title} <span className="text-sm font-normal text-text-muted">({items.length})</span></h2>{items.length ? <div className="divide-y">{items.map((item, index) => <div key={index} className="px-5 py-3 text-sm [&_a]:block [&_a]:rounded-md [&_a]:outline-none [&_a:focus]:ring-2 [&_a:focus]:ring-primary [&_span]:mt-1 [&_span]:block [&_span]:text-xs [&_span]:text-text-muted">{item}</div>)}</div> : <p className="px-5 py-4 text-sm text-text-muted">No matching records.</p>}</section>; }
