...
export default async function NewLaboratoryOrderPage({ searchParams }: { searchParams: Promise<{ patientId?: string; treatmentPlanId?: string }> }) {
  ...
  return <div className="flex flex-col gap-5 pb-12">
    ...
    <header className="rounded-card border border-border bg-card p-4.5 shadow-[var(--shadow)]"><div className="flex items-start gap-4"><span className="rounded-control bg-secondary p-3 text-heading"><FlaskConical className="size-6"/></span><div><p className="text-[11px] font-semibold tracking-[0.08em] text-text-muted uppercase">Private draft</p><h1 className="mt-1 text-[22px] leading-tight font-bold text-heading">Send work to the lab</h1><p className="mt-2 max-w-3xl text-[13px] text-text-muted">Link the case to the patient, the treatment and any X-rays. A dentist approves it before the lab sees anything, and the lab only ever gets the case number and the clinical scope.</p></div></div></header>
    {laboratories.length ? <section className="rounded-card border border-border bg-card p-4.5 shadow-[var(--shadow)]"><LabOrderForm patients={patients} laboratories={laboratories} providers={providers} plans={plans} appointments={appointments.map((item) => ({ ...item, appointmentDate: item.appointmentDate.toISOString() }))} requestId={randomUUID()} initialPatientId={initialPatientId} initialPlanId={initialPlanId}/></section> : <section className="rounded-card border border-warning-border bg-warning-bg p-6"><h2 className="font-semibold text-warning">Add a lab first</h2><p className="mt-1 text-[13px] text-warning">You need a lab on file, with its data-handling set up, before you can send a case out.</p><Link href="/dashboard/laboratory" className="mt-4 inline-flex rounded-control bg-primary px-4 py-2 text-sm font-semibold text-white">Open directory</Link></section>}
