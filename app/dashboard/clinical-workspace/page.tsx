...
import { clockTime, exactStamp, rupees } from "@/lib/format";
...
export default async function ClinicalPage({
  ...
  const [inChair, todaysVisits, matches, matchCount] = await Promise.all([
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        title="Clinical"
        sub="Chart or prescribe. Nothing here waits on a visit being marked off."
      />
      ...
      {searching && (
        ...
        >
          {matches.length === 0 ? (
            <div className="flex flex-col items-center gap-1.5 px-5.5 pt-6 pb-8 text-center">
            ...
            <>
              {matches.map((patient) => {
                ...
                return (
                  <div
                    key={patient.id}
                    className="grid items-center gap-3 border-t border-border px-5.5 py-2.5 sm:grid-cols-[minmax(0,1fr)_170px]"
                  >
              ...
              {matchCount > matches.length && (
                <p className="border-t border-border px-5.5 py-3 text-xs text-text-muted">
        ...
        inChair.map((encounter) => {
          ...
          return (
            <section
              ...
              className="rounded-card border border-border-strong bg-card px-5.5 py-4 shadow-[var(--shadow)]"
            >
          ...
          upcoming.slice(0, UPCOMING_LIMIT).map((visit) => (
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <p className="text-[length:var(--text-body)] leading-[var(--text-body-lh)] text-text-muted">{visit.treatment}</p>
              </div>
      ...
      <SectionCard
        ...
        aside={
          seen.length === 0
            ? "Nobody seen yet"
            : `${seen.length} ${seen.length === 1 ? "patient" : "patients"} through the chair`
        }
      >
        {seen.length === 0 ? (
          ...
          seen.map((visit) => {
            const openHref = visit.patientId !== null
