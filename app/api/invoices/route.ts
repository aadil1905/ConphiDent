      ...
      try {
        invoice = await prisma.$transaction(async (tx) => {
          ...
          if (amountPaidToday > 0) {
            const payment = await tx.payment.create({ data: { clinicId: user.clinicId, encounterId: encounter.id, invoiceId: created.id, amount: amountPaidToday, method: data.paymentMethod || "Cash", paidAt: issuedAt, notes: data.paymentNotes || null, referenceNumber: data.paymentReference || null, recordedBy: user.fullName, recordedByUserId: user.id } });
