...
export default function DentalOdontogram() {
  ...
  return <section className="workspace-card w-full min-w-0 self-start p-5"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div><h2 className="text-xl font-bold leading-tight text-slate-900">Dental chart</h2><p className="mt-1 text-sm leading-5 text-slate-500">A quick clinical overview. Select a tooth for details.</p></div><span className={`rounded-full px-3 py-1 text-xs font-semibold ${selected.className}`}>Tooth {selectedTooth}: {selected.label}</span></div>
