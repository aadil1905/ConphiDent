...
export default async function PatientImagingGallery({ clinicId, patientId, patientName, canAnnotate, hideWhenEmpty = false }: { clinicId: number; patientId: number; patientName: string; canAnnotate: boolean; hideWhenEmpty?: boolean }) {
  ...
  return <section id="patient-xrays" className="scroll-mt-24 rounded-card border border-border bg-card p-5 shadow-sm sm:p-6"><div className="mb-5 flex flex-wrap items-end justify-between gap-3"><div><h2 className="flex items-center gap-2 text-xl font-bold text-heading"><FileImage className="size-5 text-primary" />Patient X-rays</h2><p className="mt-1 text-sm text-text-muted">Images remain linked to this patient and their recorded FDI teeth.</p></div>{studies.length ? <span className="rounded-full bg-primary/10 px-3 py-1 text-sm font-semibold text-primary">{studies.length} image{studies.length === 1 ? "" : "s"}</span> : null}</div><ImagingGallery studies={galleryStudies} canAnnotate={canAnnotate} emptyMessage="No X-rays have been uploaded for this patient yet." /></section>;
}
