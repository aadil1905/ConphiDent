...
export function WhatsAppQuickSend({ patientId, phone, title = "WhatsApp", sourceType = "PATIENT_MESSAGE", sourceId }: { patientId: number; phone?: string | null; title?: string; sourceType?: string; sourceId: string }) {
  ...
  return <div className="relative"><ConfirmDialog open={gate.open} copy={{ title: scheduledAt ? "Schedule this message?" : "Send this on WhatsApp?", body: "Only send it if the patient told you it is fine to message this number about their care.", confirmLabel: scheduledAt ? "Schedule it" : "Send it", keepLabel: "Not now", tone: "primary" }} onConfirm={gate.confirm} onCancel={gate.cancel} /><button type="button" onClick={() => setOpen(x => !x)} className="inline-flex min-h-11 items-center gap-2 rounded-control bg-primary px-3 text-sm font-bold text-primary-foreground hover:bg-primary"><MessageCircle className="size-4" />{title}</button>{open && <div className="absolute right-0 z-30 mt-2 w-80 rounded-card border bg-card p-4 shadow-[var(--shadow-overlay)]"><p className="text-sm font-bold">Preview before sending</p><p className="mt-1 text-xs text-text-muted">To {phone}</p><textarea value={message} onChange={event => setMessage(event.target.value)} placeholder="Write a custom message…" className="mt-3 min-h-28 w-full rounded-control border p-3 text-sm"/><label className="mt-3 block text-xs font-semibold text-text-muted">Schedule delivery (optional)<input type="datetime-local" value={scheduledAt} onChange={event => setScheduledAt(event.target.value)} className="mt-1 min-h-11 w-full rounded-control border px-3 text-sm"/></label><button type="button" disabled={sending || !message.trim()} onClick={() => gate.ask(() => void send())} className="mt-3 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-control bg-primary text-sm font-bold text-primary-foreground disabled:opacity-60"><Send className="size-4" />{sending ? "Saving…" : scheduledAt ? "Confirm & schedule" : "Confirm & send"}</button></div>}</div>;
}
